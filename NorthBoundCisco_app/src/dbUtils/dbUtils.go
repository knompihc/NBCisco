...
func CreateUserTable(DbUtilsStructPtr *DbUtilsStruct) {
	...
	qStatement = "CREATE TABLE IF NOT EXISTS `user` (`id` bigint(20) NOT NULL AUTO_INCREMENT,`email` varchar(45) NOT NULL,`pass` blob NOT NULL,`isverified` int(11) NOT NULL DEFAULT 0,`verikey` varchar(500) NOT NULL,`mob_num` varchar(45) NOT NULL,`netpassphrase` varchar(500) NOT NULL,`name`	varchar(45),PRIMARY KEY (`id`))"
	DbUtilsStructPtr.CreateDBTable(qStatement)
...
func CreateSGUTable(DbUtilsStructPtr *DbUtilsStruct) {
	...
	qStatement = "CREATE TABLE IF NOT EXISTS sgu ( sgu_id bigint PRIMARY KEY," +
		"location_name varchar(45) NOT NULL DEFAULT ' ',location_lat double precision, location_lng double precision)"
...
func CreateSCUTable(DbUtilsStructPtr *DbUtilsStruct) {
	...
	qStatement = "CREATE TABLE IF NOT EXISTS scu ( scu_id bigint PRIMARY KEY," +
		...
		"location_name varchar(45),location_lat double precision, location_lng double precision)"
...
func (DbUtilsStructPtr *DbUtilsStruct) DbUtilsInit(logg *log.Logger) {
	...
	if runtime.GOOS == "windows" {
		...
		if !useRemoteDB {
			...
			dbinfo = "root:admin123@tcp(localhost:3306)/HavellsCCMSDatabase"

		} else {

			//db in oragon
			//dbinfo = "--host=mysql.cqwf1pvghoch.us-west-2.rds.amazonaws.com --user=HavellsDBAdmin --database=HavellsCCMSDatabase  --password=HavellsCCMS420"
