...
const (
	...
	sguCheckResponseTimeInMiliseconds=1000*3*60
...
func (SguUtilsStructPtr *SguUtilsStruct)SGUGetLampStatus() {
	...
	sguIndex, _ := 	IsSGUinRamList(SguUtilsStructPtr)
...
func (SguUtilsStructPtr *SguUtilsStruct)SendAlertSMS() {
	...
	if sgu_name==" "||len(sgu_name)==0{
		SMSstring += "SGUID=" + strconv.FormatUint(SguUtilsStructPtr.SGUID,16) + "%20"
	}else{
		SMSstring += "SGU%20" + sgu_name + "%20,ID=" + strconv.FormatUint(SguUtilsStructPtr.SGUID,16) + "%20"
	}
